...
def load_model():
    model = (
        AutoModelForCausalLM.from_pretrained("gongxiucheng/internlm-chat-7b-for-recipe", trust_remote_code=True)
        .to(torch.bfloat16)
        .cuda()
    )
    tokenizer = AutoTokenizer.from_pretrained("gongxiucheng/internlm-chat-7b-for-recipe", trust_remote_code=True)
